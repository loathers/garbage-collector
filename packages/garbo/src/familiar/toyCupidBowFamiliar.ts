...
export function getToyCupidBowFamiliars(): GeneralFamiliar[] {
  const fam = Familiar.all().filter((f) => {
    ...
    return equipment.tradeable === true && have(f) && !ToyCupidBow.familiarsToday().includes(f);
  });
  ...
  return [{
        ...
        worksOnFreeRun: true,
        expectedValue: estimatedGarboTurns() >= 5 ? garboValue(familiarEquipment(bestFamiliar)) / 5 : 0,
