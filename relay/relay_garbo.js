...
function main() {
  ...
  var fields = kolmafia.formFields();
  Object.keys(fields).forEach(field => {
    if (field.includes("_didchange")) return;
    if (field === "relay") return;
    var oldSetting = kolmafia.formField(`${field}_didchange`);
  ...
  var settings = JSON.parse(kolmafia.fileToBuffer("garbo_settings.json"));
  ...
  kolmafia.writeln('<head><link rel="stylesheet" href="/garbage-collector/index.css"></head>');
  kolmafia.writeln('<div id="root"></div>');
  kolmafia.writeln("<script>");

  // add script that react calls when loaded to get kol data
  kolmafia.writeln(`window.getData = function(callback) {callback(${escapeUnsafeChars(JSON.stringify({
    settings: settings,
    updatedSettings: updatedSettings
  }))})}`);

  // close notifications when they are clicked on
  kolmafia.writeln(`document.onclick = (e) => {
    if(e.target.classList.contains('notification')) e.target.remove();
  }`);
  kolmafia.writeln("</script>");

  // include react scripts
  kolmafia.writeln('<script src="./garbage-collector/index.js"></script>');
}
